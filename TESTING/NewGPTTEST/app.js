        ...
        const animate = () => {
            ...
            columns.forEach((column, index) => {
                if (!allStopped[index]) {
                    ...
                    if (shouldStop) {
                        ...
                        // **Spring-back effect**
                        setTimeout(() => {
                            column.style.transform = `translateY(${itemHeight + 15}px)`;
                            setTimeout(() => {
                                column.style.transform = `translateY(${itemHeight}px)`;
                            }, 150);
                        }, 100);
    
                        // **Glow effect on final image**
                        const selectedItem = column.children[0];
                        if (selectedItem) {
                            selectedItem.classList.add("selected-glow");
                        }
                    }
            ...
            if (animationRunning) {
                ...
                callback(columns.map((col) => {
                    const selectedItem = col.querySelector(".selected-glow");
