// Initialize state OUTSIDE the DOMContentLoaded event listener
let state = {
    selectedClass: null,
    isSpinning: false,
    currentSpin: 1,
...
// Loadouts object - Define outside since it's constant
const loadouts = {
    Light: {
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    console.log("Found elements:", {
        classButtons: classButtons.length,
        spinButtons: spinButtons.length,
        spinSelection: !!spinSelection,
        outputDiv: !!outputDiv
    });
    ...
    const displayLoadout = (classType, loadout) => {
        ...
        const gadgets = getUniqueGadgets(classType, loadout);
    ...
    const spinLoadout = (spins) => {
        console.log("spinLoadout called with spins:", spins);
        ...
        if (state.selectedClass === 'random') {
            console.log("Displaying random loadout");
            displayRandomLoadout();
        } else {
            console.log("Displaying manual loadout for class:", state.selectedClass);
            displayManualLoadout(state.selectedClass);
        }
    };

    // Add GPU hints to columns for better performance
    const addGPUHints = () => {
    ...
    // Call GPU hints once on load
    addGPUHints();
}); // <-- Single closing brace for the original DOMContentLoaded